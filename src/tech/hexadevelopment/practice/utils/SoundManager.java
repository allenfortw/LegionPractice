...
public class SoundManager {
	...
	public static void playSound(Player p, Location location, String sound, float i, float f, boolean place) {
		...
		if(!plugin.getNMSAccessProvider().getVersion().startsWith("v1_7_R")
				&& !plugin.getNMSAccessProvider().getVersion().startsWith("v1_8_R")) {
		...
		if(s == null) {
			Bukkit.getLogger().warning("LegionPractice >> Couldn't find sound '" + sound + "'");
		}
		else {
